...
class Setup_WP_App_In_Console_Action_Test extends Unit_Test_Case {
	...
	public function test_perform_setup_actions(): void {
		// Expect comments and calls for publishing Laravel and Enpii Base migrations and assets
		...
		$this->console_command->shouldReceive( 'comment' )
			->with( 'Publishing Enpii Base Migrations...' )
			->once();
		$this->console_command->shouldReceive( 'call' )
			->with(
				'vendor:publish',
		...
		$this->console_command->shouldReceive( 'comment' )
			->with( 'Publishing Enpii Base Assets...' )
			->once();
		$this->console_command->shouldReceive( 'call' )
			->with(
				'vendor:publish',
